...
void rayleightable(double emin = 110, double emax = 700, int nsteps = 500, int index = 0) {
    const double minlambda = 110;
    const double maxlambda = 700;
    if (emin < minlambda || emax > maxlambda) {
        cout << " variables out of range: " << minlambda << " - " << maxlambda << endl;
...
void spectrum() {
    TF1* f1 = new TF1("f1", "ROOT::Math::normal_pdf(x, 10, 128) ", 100, 160);
    f1->SetLineWidth(2);
    f1->SetLineColor(2);
    f1->GetXaxis()->SetTitle("#lambda [nm]");
    f1->GetYaxis()->SetTitle("Intensity [Arbitrary Units]");
    f1->SetMinimum(0.);
    f1->Draw();
}
